...
public class MapManager : MonoBehaviour
{
            ...
            for (int j = 0; j < Constants.mapWidth; j++)
            {
                ...
                if (map[idx] != (int)tileType.water)
                {
                    if (Random.Range(0, 1f) > 0.5f)
                        tilemaps[0].SetTile(pos, map_tiles[0].arr[4]);
                    else
                        tilemaps[0].SetTile(pos, map_tiles[0].arr[5]);
                    if (map[idx] != (int)tileType.baseTile)
                        tilemaps[1].SetTile(pos, map_tiles[0].arr[map[idx]]);
                }
                else
                {
                    tilemaps[0].SetTile(pos, map_tiles[0].arr[3]);
                }
            }
        }
    }
    ...
    IEnumerator effect(int x, int y, int version)
    {
        ...
        while (true)
        {
            idx %= map_tiles[2].size;
            tilemaps[4].SetTile(new Vector3Int(x, y, 0), map_tiles[version * 2 + 1].arr[idx]);
            tilemaps[3].SetTile(new Vector3Int(x, y, 0), map_tiles[version * 2 + 2].arr[idx++]);
            yield return new WaitForSeconds(0.2f);
        }
    }

    void update_tileanims(int pos, int version)
    {
        int x = pos / 5;
        int y = pos % 5;
        ...
        if (StageManager.stageManager.player.character_Stat.moveType == moveType.ground)
        {
            ...
            while (q.Count != 0)
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    ...
                    if (xx < 0 || xx >= Constants.mapHeight || yy < 0 || yy >= Constants.mapWidth) continue;
    ...
    private List<Node> GetNeighborList(Node currNode, Node[,] mapp)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            if (nx >= 0 && nx < Constants.mapHeight && ny >= 0 && ny < Constants.mapWidth)
